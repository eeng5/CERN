...
class Model(tf.keras.Model):
    def __init__(self):
        ...
        self.layer4 = tf.keras.layers.Dense(160, activation="tanh")
        self.layer5 = tf.keras.layers.Dense(128, activation="relu")
        self.layer6 = tf.keras.layers.Dense(64, activation="relu")
        self.layer7 = tf.keras.layers.Dense(32, activation="tanh")
...
def main():
    ...
    for i in range(150):
        ...
        if i == 149:
            np.savetxt('tau_unified_model.csv', np.array(tau_res), delimiter=',')
            np.savetxt('antitau_unified_model.csv',np.array(antitau_res), delimiter=',')
