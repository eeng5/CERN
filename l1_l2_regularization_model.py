...
class Model(tf.keras.Model):
    def __init__(self):
        ...
        #L1 and L2 are two forms of regularization used in machine learning to minimize the error of the loss function.
        self.regularizers = tf.keras.regularizers.l1_l2(l1=0.01, l2=0.01)
        self.optimizer = tf.keras.optimizers.Adam(learning_rate=0.002, beta_1=0.5)
        self.layer1 = tf.keras.layers.Dense(640, activation="relu", kernel_regularizer=self.regularizers)
        self.layer2 = tf.keras.layers.Dense(640, activation="relu", kernel_regularizer=self.regularizers)
        self.layer3 = tf.keras.layers.Dense(320, activation="relu", kernel_regularizer=self.regularizers)
        self.layer4 = tf.keras.layers.Dense(160, activation="relu", kernel_regularizer=self.regularizers)
        self.layer5 = tf.keras.layers.Dense(128, activation="relu", kernel_regularizer=self.regularizers)
        self.layer6 = tf.keras.layers.Dense(64, activation="relu", kernel_regularizer=self.regularizers)
        self.layer7 = tf.keras.layers.Dense(32, activation="relu", kernel_regularizer=self.regularizers)
        self.layer8 = tf.keras.layers.Dense(8, activation="relu", kernel_regularizer=self.regularizers)
        self.layer9 = tf.keras.layers.Dense(3, activation="relu", kernel_regularizer=self.regularizers)
